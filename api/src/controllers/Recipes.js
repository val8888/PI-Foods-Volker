    ...
    try {
        if (!name) {
            ...
            if (!recipeDB.length & !recipeApi.length) {
                return res
                    .status(404)
                    .send('There are no results in the search');
            }
...
const getRecipeById = async (req, res) => {
    ...
    try {
        if (id.includes('-')) {
            let aaaa = await Recipe.findByPk(id, { include: Diet });
        ...
        return res
            .status(404)
            .send('There are no results in the search', error);
    }
};

const createRecipe = async (req, res) => {
    const { title, img, diets, summary, healthScore, analyzedInstructions } =
        req.body;

    if (!title || !summary)
        return res.status(404).send('The title and summary are required.');
